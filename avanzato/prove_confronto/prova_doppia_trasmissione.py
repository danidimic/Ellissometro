...
for i in range(nvalues):
	...
	h1 = covariance_vector(H1)
	h2 = covariance_vector(H2)
    ...
    #quaterione finale del raggio attraverso il singolo materiale
...
plt.plot(theta_vect, S , label='I')
plt.plot(theta_vect, Q , label='Q')
plt.plot(theta_vect, U , label='U')
plt.plot(theta_vect, V , label='V')
plt.grid(True)
plt.legend()
plt.show()
...
plt.title("Calcolo tramite quaternioni")
plt.xlabel("Angolo incidente [rad]")
plt.plot(theta_vect, Psi, label='$\Psi$ quaternioni')
plt.plot(theta_vect, Delta, label='$\Delta$ quaternioni')
plt.grid(True)
plt.legend()
plt.show()
'''
